...
{
    public class AddWorkSampleTests : ResumeServiceTests
    {
        [Fact]
        public async Task Should_Throw_Exception_When_User_Cannot_Be_Found()
        {
            ...
            //Act
            var act = async () => await _resumeWiteService.AddWorkSampleAsync(command, Guid.NewGuid());
            act.Invoke();
            ...
            A.CallTo(() => _unitOfWork.WorkSampleRepository.AddAsync(A<WorkSample>._)).MustNotHaveHappened();
        ...
        public async Task Should_Create_Resume_If_It_Does_Not_Exist()
        {
            ...
            A.CallTo(() => _unitOfWork.WorkSampleRepository.AddAsync(A<WorkSample>._)).MustHaveHappenedOnceExactly();
